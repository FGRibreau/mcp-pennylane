...
- ~72 essential tools exposed directly: customers, suppliers, customer/supplier invoices, products, quotes, banking, ledger, exports, file attachments, changelogs, GoCardless and SEPA mandates.
...
- stdio transport (default) and streamable-http transport (`--transport http`).
...
- OpenAPI spec vendored at `openapi/accounting.json`; `cargo run -p refresh-openapi` to refresh.
